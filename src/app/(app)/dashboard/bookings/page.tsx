...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
  booked:    { label: "Booked",    color: "bg-brand/20 text-brand border border-brand/30" },
  completed: { label: "Completed", color: "bg-success/20 text-success border border-success/30" },
  cancelled: { label: "Cancelled", color: "bg-danger/20 text-danger border border-danger/30" },
  "no-show": { label: "No-show",   color: "bg-warning/20 text-warning border border-warning/30" },
};
    ...
    <>
        ...
        actions={
          <>
            <div className="flex h-9 rounded-xl border border-line bg-white/[0.03] overflow-hidden">
              <button onClick={() => setView("calendar")} className={cn("px-3 flex items-center gap-1.5 text-small font-medium cursor-pointer transition-colors", view === "calendar" ? "bg-white/[0.06] text-fg" : "text-fg-muted hover:text-fg")}>
                <LayoutGrid className="w-3.5 h-3.5" /> Calendar
              </button>
              <button onClick={() => setView("list")} className={cn("px-3 flex items-center gap-1.5 text-small font-medium cursor-pointer border-l border-line transition-colors", view === "list" ? "bg-white/[0.06] text-fg" : "text-fg-muted hover:text-fg")}>
        ...
        <Card className="!p-0 overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[680px]">
              ...
              {/* Hour rows */}
              {HOURS.map((h) => (
                <div key={h} className="grid grid-cols-[56px_repeat(7,1fr)] border-b border-line" style={{ minHeight: "56px" }}>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-small">
                  ...
                  <tbody className="divide-y divide-line">
                    {filtered.map((b) => (
                      <tr key={b.id} className="hover:bg-white/[0.02] cursor-pointer transition-colors" onClick={() => setActive({ ...b, _isGcal: false })}>
          ...
          <aside className="fixed inset-x-0 bottom-0 z-50 max-h-[90vh] overflow-y-auto bg-ink-900 rounded-t-2xl border-t border-line scrollbar-thin animate-slide-up md:inset-auto md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:w-full md:max-w-md md:max-h-[88vh] md:rounded-2xl md:border md:shadow-pop">
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => { setActive(null); setEditingGcal(false); }} className="w-8 h-8 flex items-center justify-center rounded-lg text-fg-muted hover:text-fg hover:bg-white/[0.04] cursor-pointer transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>
                  ...
                  {!editingGcal ? (
                    <>
                      ...
                      <button
                        ...
                        className="w-full h-10 rounded-xl bg-white/[0.04] border border-line flex items-center justify-center gap-2 text-small font-medium text-fg hover:bg-white/[0.08] cursor-pointer transition-colors"
                      >
                      ...
                      <div className="space-y-3">
                        <ModalField label="Title">
                          <input
                            ...
                            className="w-full h-10 px-3 rounded-xl bg-ink-800 border border-line text-small text-fg outline-none focus:border-brand/50 transition-colors"
                          />
                        </ModalField>
                ...
                <>
                  <div className="text-center py-2">
                    ...
                    <div className="mt-2 flex justify-center">
                      <span className={cn("inline-flex items-center px-2.5 py-0.5 rounded-lg text-tiny font-semibold capitalize", STATUS_CONFIG[activeBooking.status]?.color ?? "bg-white/[0.06] text-fg-muted border border-line")}>
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            activeBooking.status === s
                              ? "bg-white/[0.12] text-fg border border-white/20"
                              : "bg-white/[0.03] text-fg-muted border border-line hover:text-fg hover:bg-white/[0.07]"
                          )}
                        >
                          {s}
                        </button>
      ...
      {showAdd && (
        <>
          ...
          <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-md bg-ink-900 border border-line rounded-2xl shadow-pop animate-slide-up flex flex-col overflow-hidden" style={{ maxHeight: "90vh" }}>
            ...
            <div className="space-y-3">
              ...
              <ModalField label="Customer name *">
                <input value={addForm.customer_name} onChange={(e) => setAddForm({ ...addForm, customer_name: e.target.value })} placeholder="e.g. John Smith" className="w-full h-9 px-3 rounded-xl bg-ink-800 border border-line text-small text-fg placeholder:text-fg-faint outline-none focus:border-brand/50 transition-colors" />
              </ModalField>

              {/* Row 2: Phone + Email */}
              <div className="grid grid-cols-2 gap-3">
                <ModalField label="Phone">
                  <input value={addForm.customer_phone} onChange={(e) => setAddForm({ ...addForm, customer_phone: e.target.value })} placeholder="+27 82 123 4567" className="w-full h-9 px-3 rounded-xl bg-ink-800 border border-line text-small text-fg placeholder:text-fg-faint outline-none focus:border-brand/50 transition-colors" />
                </ModalField>
                <ModalField label="Email">
                  <input type="email" value={addForm.customer_email} onChange={(e) => setAddForm({ ...addForm, customer_email: e.target.value })} placeholder="optional" className="w-full h-9 px-3 rounded-xl bg-ink-800 border border-line text-small text-fg placeholder:text-fg-faint outline-none focus:border-brand/50 transition-colors" />
                </ModalField>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <ModalField label="Date *">
                  <div className="flex items-center gap-1">
                    <button type="button" onClick={() => { const d = addForm.date ? new Date(addForm.date + "T12:00:00") : new Date(); d.setDate(d.getDate() - 1); setAddForm({ ...addForm, date: d.toISOString().split("T")[0] }); }} className="w-7 h-9 flex items-center justify-center rounded-lg bg-ink-800 border border-line text-fg-muted hover:text-fg cursor-pointer transition-colors shrink-0"><ChevronLeft className="w-3.5 h-3.5" /></button>
                    <input type="date" value={addForm.date} onChange={(e) => setAddForm({ ...addForm, date: e.target.value })} className="flex-1 h-9 px-2 rounded-xl bg-ink-800 border border-line text-small text-fg outline-none focus:border-brand/50 transition-colors text-center min-w-0" />
                ...
                <ModalField label="Time">
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="w-7 h-9 flex items-center justify-center rounded-lg bg-ink-800 border border-line text-fg-muted hover:text-fg cursor-pointer transition-colors shrink-0"
                    >
                    ...
                    <input
                      ...
                      className="flex-1 h-9 px-2 rounded-xl bg-ink-800 border border-line text-small text-fg outline-none focus:border-brand/50 transition-colors text-center min-w-0"
                    />
                    <button
                      type="button"
                      onClick={() => {
              ...
              <div className="grid grid-cols-2 gap-3">
                <ModalField label="Job type">
                  <input value={addForm.job_type} onChange={(e) => setAddForm({ ...addForm, job_type: e.target.value })} placeholder="e.g. Geyser repair" className="w-full h-9 px-3 rounded-xl bg-ink-800 border border-line text-small text-fg placeholder:text-fg-faint outline-none focus:border-brand/50 transition-colors" />
                </ModalField>
                <ModalField label="Area">
                  <input value={addForm.area} onChange={(e) => setAddForm({ ...addForm, area: e.target.value })} placeholder="e.g. Sandton" className="w-full h-9 px-3 rounded-xl bg-ink-800 border border-line text-small text-fg placeholder:text-fg-faint outline-none focus:border-brand/50 transition-colors" />
                </ModalField>
              </div>
              ...
              <ModalField label="Price">
                <div className="space-y-2">
                    ...
                    {PRICE_TYPES.map((pt) => (
                      <button
                        ...
                        className={cn(
                          ...
                          addForm.priceType === pt ? "bg-brand text-white" : "bg-ink-800 text-fg-muted hover:text-fg"
                        )}
                      >
                        {PRICE_LABELS[pt]}
                      </button>
                  ...
                  {addForm.priceType === "fixed" && (
                    <input type="number" min="0" value={addForm.priceMin} onChange={(e) => setAddForm({ ...addForm, priceMin: e.target.value })} placeholder="Amount e.g. 1500" className="w-full h-9 px-3 rounded-xl bg-ink-800 border border-line text-small text-fg placeholder:text-fg-faint outline-none focus:border-brand/50 transition-colors" />
                  )}
                  {addForm.priceType === "range" && (
                    <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 w-full">
                      <input type="number" min="0" value={addForm.priceMin} onChange={(e) => setAddForm({ ...addForm, priceMin: e.target.value })} placeholder="From" className="min-w-0 w-full h-9 px-3 rounded-xl bg-ink-800 border border-line text-small text-fg placeholder:text-fg-faint outline-none focus:border-brand/50 transition-colors" />
                      <span className="text-tiny text-fg-subtle">to</span>
                      <input type="number" min="0" value={addForm.priceMax} onChange={(e) => setAddForm({ ...addForm, priceMax: e.target.value })} placeholder="To" className="min-w-0 w-full h-9 px-3 rounded-xl bg-ink-800 border border-line text-small text-fg placeholder:text-fg-faint outline-none focus:border-brand/50 transition-colors" />
                    </div>
                  )}
                </div>
              </ModalField>
              ...
              <ModalField label="Job notes">
                <textarea
                  ...
                  rows={2}
                  className="w-full px-3 py-2 rounded-xl bg-ink-800 border border-line text-small text-fg placeholder:text-fg-faint outline-none focus:border-brand/50 transition-colors resize-none"
                />
              </ModalField>
            </div>
