...
export default function Home() {
  ...
  return (
    <>
      ...
      {/* ═══════ 02b · SOCIAL PROOF ══════════════════════════════ */}
      {/*
        Placeholder section: supervisor will swap the silhouette logos for
        real client marks and replace {TESTIMONIAL_PLACEHOLDER} with a real
        quote, attribution, and avatar before launch.
      */}
      <section className="relative py-28 md:py-40 overflow-hidden">
        <div className="relative mx-auto max-w-site px-6 lg:px-10">
          <div className="text-center max-w-3xl mx-auto">
            <p className="eyebrow text-ink-500 mb-6 reveal-up">Social proof</p>
            <h2 className="display-lg text-ink reveal-up">
              Trusted by service businesses{" "}
              <em className="italic font-light">across South Africa</em>.
            </h2>
          </div>

          {/* Logo wall (silhouettes — supervisor to replace with real marks) */}
          <ul
            aria-label="Customer logos"
            className="mt-16 md:mt-20 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-x-8 gap-y-10 items-center justify-items-center reveal-stagger"
          >
            {[
              "Atlas Plumbing",
              "Northbeach Salon",
              ...
              >
                <svg
                  ...
                  viewBox="0 0 160 48"
                  className="h-10 w-auto text-ink-500"
                  fill="none"
                >
                  <rect x="0" y="14" width="20" height="20" rx="4" fill="currentColor" />
                  <rect x="28" y="18" width="56" height="6" rx="3" fill="currentColor" />
                  <rect x="28" y="28" width="80" height="4" rx="2" fill="currentColor" opacity="0.55" />
                </svg>
              </li>
            ))}
          </ul>
          ...
          {/* Testimonial card */}
          <figure className="mt-20 md:mt-24 mx-auto max-w-2xl reveal-up">
            <div className="ed-card relative px-8 md:px-10 py-10 md:py-12 text-center">
              <span
                aria-hidden="true"
                className="font-display text-ember/40 text-7xl leading-none block mb-2"
              >
                &ldquo;
              </span>
              ...
              <figcaption className="mt-8 flex items-center justify-center gap-3 text-sm text-ink-500">
                <span className="w-9 h-9 rounded-full bg-ink/10 border border-ink/15" aria-hidden="true" />
                <span className="text-left">
                  <span className="block text-ink font-medium">{`{NAME_PLACEHOLDER}`}</span>
                  <span className="block text-ink-500">{`{ROLE_PLACEHOLDER}`}</span>
                </span>
              </figcaption>
            </div>
          </figure>
