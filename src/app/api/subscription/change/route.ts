...
export async function POST(req: NextRequest) {
  ...
  // Downgrade to Starter = cancel
  if (plan === "starter") {
    return NextResponse.json({ error: "use /api/subscription/cancel to downgrade to Starter" }, { status: 400 });
  }
  ...
  if (!customerCode) {
    try {
      const customer = await paystack.customer.create({
        ...
        last_name: biz.name?.split(" ").slice(1).join(" ") || undefined,
      });
      ...
      console.error("[subscription/change] customer create error:", err);
  ...
  if (auth.paystackSubscriptionCode && auth.paystackEmailToken) {
    try {
      ...
      console.warn("[subscription/change] disable old subscription failed:", err);
      // Non-fatal — continue with new subscription
    }
  }

  // Initialize Paystack transaction (creates subscription after payment)
  try {
    ...
    console.error("[subscription/change] transaction initialize error:", err);
    return NextResponse.json({ error: "billing_provider_error" }, { status: 502 });
  }
}
