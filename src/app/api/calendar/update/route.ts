...
export async function PATCH(req: NextRequest) {
  const { clientId, eventId, calendarId: calId, title, startDateTime, endDateTime } = await req.json() as {
    clientId: string;
    eventId: string;
    calendarId?: string;
    title: string;
  ...
  if (!clientId || !eventId || !title || !startDateTime || !endDateTime) {
  ...
  try {
    ...
    console.error("[calendar/update]", { clientId, eventId, error: message });
