...
export async function GET() {
  ...
  const [clientsRes, invoicesRes, periodsRes, disputesRes] = await Promise.all([
    db.from("clients").select("id, onboarding_complete, onboarding_completed_at, risk_score"),
  ...
  return NextResponse.json({
    ...
    active_clients: clients.filter(c => c.onboarding_complete === true || !!c.onboarding_completed_at).length,
