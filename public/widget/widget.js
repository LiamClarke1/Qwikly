/* Qwikly Embed Widget v5.0 — qwikly.co.za/embed.js */
(function () {
  "use strict";

  var script = document.currentScript || document.querySelector("script[data-qwikly-id],script[data-client]");
  // Accept data-qwikly-id (new) or data-client (legacy backward-compat)
  var TENANT_ID = script && (script.getAttribute("data-qwikly-id") || script.getAttribute("data-client"));
  if (!TENANT_ID) return;
  ...
  style.textContent = [
    ...
    ".cin{display:flex;align-items:flex-end;gap:8px;padding:" + CIN_PAD + ";border-top:1px solid #F1F5F9;flex-shrink:0;background:#fff}",
    ".cinp{flex:1;padding:" + INPUT_PAD + ";border:1.5px solid #E2E8F0;border-radius:14px;font-size:" + INPUT_FONT + "px;outline:none;resize:none;color:#1F2937;font-family:inherit;line-height:1.4;max-height:" + INPUT_MAXH + "px;overflow-y:auto;touch-action:manipulation;-webkit-text-size-adjust:100%}",
    ...
    ".sndbtn{width:" + BTN_SIZE + "px;height:" + BTN_SIZE + "px;border:none;border-radius:12px;background:var(--qc,#E85A2C);color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:opacity .15s;touch-action:manipulation}",
    ...
    ".attbtn{width:" + BTN_SIZE + "px;height:" + BTN_SIZE + "px;border:none;border-radius:12px;background:#F1F5F9;color:#64748B;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:" + MICRO + ";touch-action:manipulation}",
    ...
    "#nudge{position:fixed;bottom:96px;right:28px;z-index:2147483646;max-width:248px;background:#fff;color:#0F172A;border-radius:14px;padding:11px 10px 11px 14px;box-shadow:0 14px 36px rgba(15,23,42,.16),0 2px 8px rgba(15,23,42,.06);font-size:14px;line-height:1.4;font-weight:500;letter-spacing:-.01em;display:flex;align-items:center;gap:6px;opacity:0;transform:translateY(6px) scale(.96);pointer-events:none;transition:" + (prefersReduced ? "none" : "opacity .25s ease, transform .25s ease") + "}",
    "#nudge.open{opacity:1;transform:translateY(0) scale(1);pointer-events:auto}",
    "#nudge-body{flex:1;cursor:pointer;padding:1px 0}",
    "#nudge-x{background:none;border:none;color:#94A3B8;cursor:pointer;font-size:18px;line-height:1;width:24px;height:24px;display:flex;align-items:center;justify-content:center;border-radius:50%;flex-shrink:0;padding:0;transition:" + MICRO + "}",
    "#nudge-x:hover{background:#F1F5F9;color:#475569}",
    "#nudge::after{content:\"\";position:absolute;bottom:-5px;right:38px;width:12px;height:12px;background:#fff;transform:rotate(45deg);box-shadow:3px 3px 6px rgba(15,23,42,.04)}",
    // Mobile
    "@media(max-width:600px){#launcher{bottom:max(20px,calc(env(safe-area-inset-bottom) + 16px));right:max(20px,calc(env(safe-area-inset-right) + 12px));padding:14px 24px;font-size:15px}#panel{left:8px;right:8px;width:auto;bottom:max(72px,calc(env(safe-area-inset-bottom) + 68px));height:auto;max-height:60vh;border-radius:18px;transition:none}#nudge{right:max(20px,calc(env(safe-area-inset-right) + 12px));bottom:max(78px,calc(env(safe-area-inset-bottom) + 74px));max-width:220px;font-size:13px}}",
    // Dark mode
  ...
  // Pre-warm color and label from cache so the launcher renders instantly.
  ...
  function getDocIcon(mime) { return isImageMime(mime) ? IMG_SVG : FILE_SVG; }
  ...
  function addDocCard(side, docId, filename, fileSize, pending) {
    ...
    if (pending) {
      ...
      if (docId) dlBtn.addEventListener("click", function () { handleDownload(docId, dlBtn); });
    }
  ...
  // POPIA consent notice — shown once per browser, dismissed persistently.
  var CONSENT_LS_KEY = "qwikly_consent_dismissed_v1";
  function showConsentNotice() {
    try { if (localStorage.getItem(CONSENT_LS_KEY) === "1") return; } catch (_e) {}
    var m = msgsEl(); if (!m) return;
    ...
    x.addEventListener("click", function () {
      try { localStorage.setItem(CONSENT_LS_KEY, "1"); } catch (_e) {}
    ...
    fetch(API_BASE + "/api/web/availability?tenantId=" + encodeURIComponent(TENANT_ID))
      ...
      .then(function (out) {
        if (!out.ok || !out.body || !out.body.ok) {
          renderPickerUnavailable(card, out.body && out.body.reason); return;
        }
  ...
  function renderPickerUnavailable(card, reason) {
    var msg = "Couldn't load times right now. Email us and we'll book you in.";
    if (reason === "calendar_not_connected" || reason === "calendar_disconnected") {
      msg = "Booking calendar isn't set up yet. Email us directly.";
    }
    card.innerHTML = "";
    var h = document.createElement("div"); h.className = "bp-h"; h.textContent = "Book a call"; card.appendChild(h);
  ...
  function renderPickerStepDay(card) {
    ...
    var s = document.createElement("div"); s.className = "bp-sub"; s.textContent = "30 min · Google Meet"; card.appendChild(s);
    var days = groupSlotsByDay(pickerSlots);
    if (days.length === 0) {
      var empty = document.createElement("div"); empty.className = "bp-empty";
      empty.textContent = "No open days right now. Email us and we'll find one.";
  ...
  function renderPickerStepDetails(card, day, slot) {
    ...
    nameInp.autocomplete = "name"; nameInp.placeholder = "Your name";
  ...
  function submitBooking(card, slot, name, email, onError) {
    fetch(API_BASE + "/api/web/bookings", {
      ...
      .then(function (out) {
        if (!out.ok || !out.body || !out.body.ok) {
          ...
          if (reason === "calendar_not_connected") return onError("Booking calendar isn't connected. Email us directly.");
          return onError("Couldn't confirm. Try again.");
        }
  ...
  function buildPanel() {
    panel.innerHTML =
      ...
      '</div>' +
      '</div>' +
      '<div class="cin"><button class="attbtn" id="qw-att" aria-label="Attach file" title="Attach file">' + ATTACH_SVG + "</button>" +
    ...
    inp.addEventListener("input", function () {
      ...
      lastInputH = sh;
      this.style.height = Math.min(Math.max(sh, 20), INPUT_MAXH) + "px";
    });
  ...
  function fetchStarterPrompts() {
    ...
    fetch(API_BASE + "/api/web/starter-prompts?tenant_id=" + encodeURIComponent(TENANT_ID), {
      ...
      .catch(function () { starterPrompts = null; });
  }

  function renderSuggestions() {
    var list = shadow.getElementById("qw-sg-list"); if (!list) return;
    list.innerHTML = "";
    if (!starterPrompts) {
      for (var i = 0; i < 4; i++) {
        var sk = document.createElement("div"); sk.className = "sg-skel"; list.appendChild(sk);
      }
      return;
    }
    for (var j = 0; j < starterPrompts.length; j++) {
      (function (text) {
        ...
        b.type = "button"; b.className = "sg-pill"; b.textContent = text;
  ...
  function showSuggestions() {
    ...
    renderSuggestions();
    var m = msgsEl(); if (m) m.scrollTop = 0;
  }
  ...
  function sendStarterPrompt(text) {
    ...
    hideSuggestions();
    var inp = shadow.getElementById("qw-inp");
    if (inp) { inp.value = text; inp.style.height = ""; }
    handleSend();
  }
  ...
  function setupNudge(delayMs) {
    setTimeout(function () {
      if (nudgeDismissed || panelOpen) return;
      ...
      setTimeout(function () { n.classList.add("open"); }, 20);
